...
"$KUBECTL" -n "$NAMESPACE" run "$POD" \
  --image=alpine:3.22 \
  --restart=Never \
  --overrides='{"spec":{"nodeName":"k3s-master","hostPID":true,"hostNetwork":true,"containers":[{"name":"loader","image":"alpine:3.22","command":["sleep","3600"],"securityContext":{"privileged":true},"volumeMounts":[{"name":"host-root","mountPath":"/host"},{"name":"image-transfer","mountPath":"/transfer"}]}],"volumes":[{"name":"host-root","hostPath":{"path":"/"}},{"name":"image-transfer","hostPath":{"path":"/tmp/signal-image-loader","type":"DirectoryOrCreate"}}]}}' \
  --image-pull-policy=IfNotPresent >/dev/null

"$KUBECTL" -n "$NAMESPACE" wait --for=condition=Ready pod/$POD --timeout=120s >/dev/null
"$KUBECTL" -n "$NAMESPACE" cp "$TAR_FILE" "$POD:/transfer/signal-image.tar"
"$KUBECTL" -n "$NAMESPACE" exec "$POD" -- /bin/sh -c \
  'chroot /host /usr/local/bin/k3s ctr -n k8s.io images import /host/tmp/signal-image-loader/signal-image.tar'
